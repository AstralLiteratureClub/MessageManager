...
public class VariablePlaceholderCollections {
	...
	public static Collection<VariablePlaceholder> toList(@NotNull CaptionVariable... captionVariables){
		return Stream.of(captionVariables).map(bet.astral.messenger.v3.cloud.placeholders.VariablePlaceholder::of).collect(Collectors.toList());
	}
	public static @NotNull Collection<VariablePlaceholder> toList(@NotNull List<CaptionVariable> captionVariables){
		return captionVariables.stream().map(bet.astral.messenger.v3.cloud.placeholders.VariablePlaceholder::of).collect(Collectors.toList());
	}
}
